 ...
 * self-reported confidence level (1–3: low/medium/high) with actual correctness.
 *
 * Maps confidence 1–3 → expected accuracy bands:
 *   1 (low)    → expects ~33% accuracy
 *   2 (medium) → expects ~66% accuracy
 *   3 (high)   → expects ~100% accuracy
 *
...
export function computeConfidenceCalibration(
    ...
    for (const [concept, items] of byConcept) {
        if (items.length < MASTERY_CONFIG.MIN_QUESTIONS_PER_CONCEPT) {
            // Exactly 1 question → emerging signal (preliminary calibration)
            if (items.length === 1) {
                ...
                signals.push({
                    ...
                    description: `Preliminary signal from 1 question — ${items[0].is_correct ? 'answered correctly' : 'answered incorrectly'} with ${avgConfidence === 3 ? 'high' : avgConfidence === 2 ? 'medium' : 'low'} confidence.`
                })
            } else {
                signals.push({
                    concept,
                    ...
                    description: `Only ${items.length} rated question${items.length === 1 ? '' : 's'} — need more data.`
                })
            }
            continue
        }
        ...
        // Map confidence 1-3 to expected accuracy: 1→0.33, 2→0.66, 3→1.0
        ...
        if (Math.abs(gap) < 0.2) {
            ...
            description = `You rated ${concept} with high confidence but got ${Math.round((1 - accuracy) * 100)}% wrong — worth revisiting.`
        } else {
            signal = 'underconfident'
            description = `You doubted ${concept} but performed well (${Math.round(accuracy * 100)}% correct) — trust your knowledge more.`
        }
...
// -------------------------------------------------------------------------------------------------
// CLUSTERED CALIBRATION — pools prerequisite-related concepts for low-data concepts
// -------------------------------------------------------------------------------------------------
 ...
 * Returns a map of concept name → distance (1 = direct neighbor, 2 = two hops, etc.)
 */
    ...
    return selfRatings.map(rating => {
        ...
        if (perf && perf.total === 1) {
            ...
            for (const [relatedConcept, distance] of related) {
                ...
                // distance=1 → weight 0.5, distance=2 → weight 0.25
